...
REFERENCE_YEAR="${REFERENCE_YEAR:-2024}"
SLEEP="${SLEEP:-0.3}"
WORKERS="${WORKERS:-7}"
ZOOM="${ZOOM:-13}"
...
sync_group() {
  ...
  shift
  local codes=("$@")
  echo "[land-worker-${group_id}] start (${#codes[@]} prefectures, sleep=${SLEEP}s)"
  for code in "${codes[@]}"; do
    echo "[land-worker-${group_id}] prefecture ${code}"
    $PY -m app.sync_cli sync-land-prices \
      --prefecture "$code" \
      --from-year "$FROM_YEAR" \
      --to-year "$TO_YEAR" \
  ...
  echo "[land-worker-${group_id}] done"
}

echo "=== parallel land prices ${FROM_YEAR}-${TO_YEAR} ==="
echo "workers=${WORKERS}, sleep=${SLEEP}s, reference_year=${REFERENCE_YEAR}"
...
for pid in "${pids[@]}"; do
  if ! wait "$pid"; then
    echo "[error] land worker pid=$pid exited non-zero" >&2
    failed=$((failed + 1))
  fi
done

if [[ "$failed" -gt 0 ]]; then
  echo "=== parallel land prices finished with $failed failed worker(s) ===" >&2
  exit 1
fi

echo "=== parallel land prices finished ==="
